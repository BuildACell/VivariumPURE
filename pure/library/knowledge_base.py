...
from pure.library.spreadsheets import load_tsv

FLAT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "flat")

LIST_OF_FLAT_FILENAMES = (
    os.path.join("ecoli_genes.tsv"),
    os.path.join("ecoli_proteins.tsv"),
    os.path.join("ecoli_environment_molecules.tsv"),
    # os.path.join("timelines_def.tsv"),
    os.path.join("media_recipes.tsv"),
    os.path.join("media", "ecoli_base.tsv"),
...
class KnowledgeBase(object):
    ...
    def __init__(self):
        ...
        self.genes = {
            gene['symbol']: gene
            for gene in self.ecoli_genes}

        self.proteins = {
            protein['geneId']: protein
            for protein in self.ecoli_proteins}
